...
class DriftAnalysis:
    ...
    def _assess_health(self):
        ...
        if self.consciousness_trend < -0.05:
            ...
            self.health_issues.append(f"Declining consciousness: {self.consciousness_trend:.3f}")
        ...
        if regression_count > 0:
            self.is_healthy = False
            self.health_issues.append(f"Phase regressions detected: {regression_count}")
...
class DriftDetector:
    ...
    def _get_commits(self, file_path: str, max_commits: int = 50) -> List[Dict]:
        ...
        output = self._run_git("log", "--format=%H|%ai|%an|%s", f"-n{max_commits}", "--", file_path)
    ...
    def analyze_file_history(self, file_path: str, max_commits: int = 50) -> DriftAnalysis:
        """
    ...
    def analyze_codebase_evolution(
        self, extensions: List[str] = [".py"], max_files: int = 50, max_commits_per_file: int = 20
    ) -> CodebaseEvolution:
        """
        ...
        for file_path in files:
            ...
            try:
                drift = self.analyze_file_history(str(rel_path), max_commits=max_commits_per_file)
                if drift.snapshots:
        ...
        if evolution.file_drifts:
            ...
            evolution.healthy_files = sum(1 for d in evolution.file_drifts.values() if d.is_healthy)
        ...
        for path, drift in evolution.file_drifts.items():
            if drift.consciousness_trend < -0.05:
                ...
                if len(drift.snapshots) >= 5:
                    ...
                    if all(
                        recent[i].consciousness <= recent[i - 1].consciousness
...
def print_drift_report(drift: DriftAnalysis):
    ...
    print(
        f"  Time span: {drift.snapshots[0].commit_date.date()} → {drift.snapshots[-1].commit_date.date()}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="V7.3 Semantic Drift Detector")
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-commits", type=int, default=30, help="Max commits to analyze per file"
    )
    parser.add_argument("--death-spirals", action="store_true", help="Detect files in death spiral")
    ...
    try:
        ...
        if args.death_spirals:
            ...
            evolution = detector.analyze_codebase_evolution(max_commits_per_file=args.max_commits)
