...
class VocabularyManager:
    ...
    def _build_complete_vocabulary(self) -> None:
        ...
        print(f"VocabularyManager: Initialized with {len(self._keyword_map)} unique keywords.")
...
class SemanticAnalyzer:
    ...
    def _calculate_cluster_metrics(self, coords_list: List[Coordinates], concept_count: int) -> SemanticResult:
...
class GeopoliticalAnalyzer:
    ...
    def analyze_entity_posture(self, entity_name: str, entity_type: str, 
        ...
        history_result = self.analyzer.analyze_concept_cluster([historical_context]) if historical_context else self.analyzer._empty_result()
        ...
        combined_coords = Coordinates(
            ...
            justice=(actions_result.coordinates.justice * 0.7) + (history_result.coordinates.justice * 0.3),
            power=(actions_result.coordinates.power * 0.7) + (history_result.coordinates.power * 0.3),
            wisdom=(actions_result.coordinates.wisdom * 0.7) + (history_result.coordinates.wisdom * 0.3)
        )
...
class ICEAnalyzer:
    ...
    def analyze_ice(self, intent_words: List[str], context_words: List[str], 
        ...
        # Calculate distances
        intent_context_dist = self.vocab.get_distance(intent_result.coordinates, context_result.coordinates)
        intent_exec_dist = self.vocab.get_distance(intent_result.coordinates, execution_result.coordinates)
        context_exec_dist = self.vocab.get_distance(context_result.coordinates, execution_result.coordinates)
        ...
        avg_dist_from_anchor = (intent_result.distance_from_anchor + 
                               context_result.distance_from_anchor + 
                               execution_result.distance_from_anchor) / 3.0
        ice_balance = max(0.0, 1.0 - (avg_dist_from_anchor / 2.0))
        
        benevolence_score = (intent_result.coordinates.love + execution_result.coordinates.love) / 2.0
    ...
    def _calculate_ice_coordinate(self, intent: SemanticResult, context: SemanticResult, 
                                 execution: SemanticResult) -> Coordinates:
        """Calculate ICE coordinate from components"""
        return Coordinates(
            love=(intent.coordinates.love + context.coordinates.love + execution.coordinates.love) / 3,
            justice=(intent.coordinates.justice + context.coordinates.justice + execution.coordinates.justice) / 3,
            power=(intent.coordinates.power + context.coordinates.power + execution.coordinates.power) / 3,
            wisdom=(intent.coordinates.wisdom + context.coordinates.wisdom + execution.coordinates.wisdom) / 3
        )
...
class DivineInvitationSemanticEngine:
    ...
    def perform_mathematical_inference(self, unknown_word: str, context_words: List[str]) -> SemanticResult:
        """Perform mathematical inference"""
        return self.inference_engine.infer_unknown_meaning(unknown_word, context_words)
    
    def perform_ice_analysis(self, intent_words: List[str], context_words: List[str], 
                            execution_words: List[str]) -> Dict:
