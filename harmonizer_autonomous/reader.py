...
class Gesture:
    ...
    def __post_init__(self):
        if self.meaning is None:
            self.meaning = Meaning(P=self.power_expression, W=self.wisdom_expression)
...
class Reader(ast.NodeVisitor):
    ...
    def read(self, source: str, path: str = "<story>") -> Story:
        ...
        for class_name, class_gestures in self._class_gestures.items():
            ...
            self.characters.append(Character(name=class_name, line=line, gestures=class_gestures))
    ...
    def _perceive_gesture(self, node) -> Gesture:
        ...
        return Gesture(name=name, line=node.lineno, power_expression=P, wisdom_expression=W)
