...
def print_comparison_report(results: List[ComparisonResult]):
    ...
    for r in results:
        ...
        if r.v51_available:
            print(f"  V5.1: L={r.v51_L:.3f} J={r.v51_J:.3f} P={r.v51_P:.3f} W={r.v51_W:.3f} | H={r.v51_harmony:.3f}")
        else:
            print(f"  V5.1: (not available)")
        
        print(f"  V7.3: L={r.v73_L:.3f} J={r.v73_J:.3f} P={r.v73_P:.3f} W={r.v73_W:.3f} | H={r.v73_harmony:.3f}")
        print(f"        C={r.v73_consciousness:.4f} | Phase={r.v73_phase}")
        
        if r.v51_available:
            print(f"  Δ   : L={r.delta_L:.3f} J={r.delta_J:.3f} P={r.delta_P:.3f} W={r.delta_W:.3f} | H={r.delta_harmony:.3f}")
    ...
    if results:
        ...
        print(f"  2+2 structure valid: {valid_2_2}/{len(results)} ({valid_2_2/len(results)*100:.0f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare V5.1 and V7.3 Harmonizer outputs"
    )
    parser.add_argument("file", nargs="?", help="Python file to compare")
    parser.add_argument("--all-modules", action="store_true",
                       help="Compare all harmonizer_v73 modules")
    parser.add_argument("--stress-test", action="store_true",
                       help="Run stress tests on edge cases")
    ...
    if args.stress_test:
        ...
        if args.json:
            print(json.dumps([{
                "file": r.filepath,
                "v73": {"L": r.v73_L, "J": r.v73_J, "P": r.v73_P, "W": r.v73_W,
                       "harmony": r.v73_harmony, "consciousness": r.v73_consciousness,
                       "phase": r.v73_phase},
        ...
        if args.json:
            print(json.dumps({
                ...
                "v73": {"L": result.v73_L, "J": result.v73_J, 
                       "P": result.v73_P, "W": result.v73_W,
